...
按 search.sources 逐源检索（**不限日期**，见下方修订）
...
## 10. 修订（2026-09-02，两次实测后）

第一次实测（run 33573304939）：当周窗口检索到 51 篇，无一过线，该节被整个省略。
第二次（run 33576322222）：加了「当周 → 90 天 → 全时段高被引」三级阶梯后确实渲染出 4 篇，
但四篇的推荐理由分别写着「对象是黄嘌呤氧化酶而非乌司他丁」「非乌司他丁」「方法邻近，但非乌司他丁」
「研究对象非乌司他丁」——全部卡在 60–65，正落在当时评分细则的「60-79：邻近问题」档。

使用者据此定了两条，本文档随之修订：

1. **这条线不设时间限制。** 点名的是一个主题，不是某一周的新闻。三级日期阶梯（含
   `window_days`）整个删除，四个源一律按各自的相关度排序检索全部年份；仍不足
   `min_papers` 时再按被引排序补检一次，同样不限日期。日期不再是筛选条件。
2. **主题必须真的对题。** 评分细则改为以「研究对象是不是主题点名的那个」为第一原则：
   换了对象一律 30–59 分，方法学再邻近也不放行；细则里写死上面三个真实反例
   （黄嘌呤氧化酶 / 酪氨酸酶 / 胆碱酯酶），和 CMC 那套细则写死电池、等离子体反例是同一手法。
   默认门槛同时由 60 提到 75。

代价是这一节可能经常为空——当该主题当期确实没有对题文献时，空着是**正确**结果，
好过塞四篇标着「非乌司他丁」的邻近文献。日志会说明卡在哪一步。

### 10.1 追加修订：主题词必须是必要条件

改完上面两条后 run 33580469421 仍然是空的：不限日期后取回 88 篇（64 + 24 高被引），
过 75 分线 **0 篇**。查日志里 Europe PMC 的实际检索式，根因浮出水面：
...
**药名和「酶抑制」「动力学参数」这些通用词是 OR 关系**——只匹配 enzyme inhibition 的文献照样
返回，药名等于可有可无。三次运行（51 篇 0 命中 / 36 篇 4 篇邻近 / 88 篇 0 命中）全部败在这里：
不是判定太严，是检索从一开始就没在找乌司他丁。

`or_join()` 对**库内主题**是对的——一个主题就是一簇相关概念，命中任一条都算沾边；
对**点名了对象的焦点主题**是错的。因此焦点线不再复用 `QueryProfile`，改用自己的 `FocusProfile`：

- `subject_terms`：对象本身的英文名与文献里实际用的同义词/别名/缩写（必填；中文主题必须给出英文检索词）
- `aspect_terms`：切面（动力学、稳定性、纯化……），可为空
- 合成的检索式：`(subject1 OR subject2) AND (aspect1 OR aspect2)`，无切面时就只有对象组

模型漏填对象组时兜底用主题原文，保证「对象必填」这条永远成立。
