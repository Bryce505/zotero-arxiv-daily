...
DeepSeek 官方 API 无 embedding 接口，因此向量化需另行解决。

> ⚠️ **实测更正（2026-08-21 冒烟测试）**：此处原判断为「语料仅约 120 篇，本地模型几秒算完」，**实测错误**。见 8.7 节实测数据 —— `local` reranker 在 GitHub runner 上约 **3.9 秒/篇**，133 篇（22 候选 + 111 语料）耗时 **8 分 42 秒**，占整次运行时长的 86%。语料规模不是瓶颈，**runner 的 CPU 才是**。
...
### 8.7 冒烟测试实测数据（2026-08-21）

用**现有上游代码**跑通一次（`main.yml` / run 32453537429 / conclusion success），验证了五项前置条件，并测出一个推翻先前判断的性能数字。

**语料侧 —— 全部符合预期或更好：**

| 指标 | 实测 |
| --- | --- |
| 有摘要的条目 | **112 篇**（缺摘要基本不存在，库很干净） |
| `["文献", "文献/**"]` 匹配 | **111 篇**（99%，漏的 1 篇在「文献」树之外） |
| DeepSeek `deepseek-v4-flash` | ✅ 15 篇 TLDR + affiliations / 35 秒 / 零报错 |
| Gmail SMTP | ✅ `Email sent successfully` |

样本路径证实 表征 与 验证 确有子分类，且语料为**中英混合**：
...
文献/项目文献/KJ103
文献/Charge variants
全柱成像毛细管等电聚焦电泳分析尤瑞克林电荷异质性方法的建立与应用  ← 中文
```

> 中英混合意味着 **embedding 模型的中文支持强弱会直接影响相似度计算**，C 阶段需单独验证。
...
| --- | ---: | ---: |
| Zotero 拉取 | 7s | 1% |
| bioRxiv 检索（22 篇） | 32s | 5% |
| **重排（embedding）** | **8 分 42 秒** | **86%** |
| LLM 抽取（15 篇） | 35s | 6% |
| 发邮件 | 12s | 2% |

约 **3.9 秒/篇**（133 篇 = 22 候选 + 111 语料）。**瓶颈不是语料规模，是 runner 的 CPU。** 外推到 P0 的候选量级：
...
配额上无问题（私有仓 2000 分钟/月，周跑一次绰绰有余），但把 86% 的算力耗在 embedding 上不合理。**待决策的三个选项**：

1. **改用 embedding API**（硅基流动 bge-m3 / 智谱等）—— 几秒完成，成本约几分钱/周。代价是多一个 key；仓库已有 `api` reranker，**改配置即可，无需写代码**
2. **缓存语料向量** —— 111 篇语料每周基本不变，向量存入仓库，每周只算候选。省掉固定的 7 分钟
3. **维持现状** —— 接受每周 20–46 分钟

推荐 **1 + 2 并行**：API 解决候选的大头，缓存消除语料的固定开销。
...
1. **边界已实测，管线中段未实测。** 预检于 2026-08-21 在 Actions 实跑（run 32489096144，跑在合并后的 `main`）：Zotero 111/112、DeepSeek 中文应答、PubMed/Europe PMC/Crossref/OpenAlex 四源均返回真实结果、Gmail SMTP 登录通过。**四个查询式检索器由此确认能解析真实 API 响应**，此前仅有桩数据覆盖。仍未端到端验证的是：聚类质量、配额取数、OA 命中率、抽取输出、渲染与群发
