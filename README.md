...
- **归簇的「主题描述加权」修复尚未经真实候选验证。** 触发这次修复的原始案例（壳聚糖酶论文误归入 HCP 分析簇）在修复后的重跑里没有再次出现——但那是因为它的 DOI 已经记在 `state/seen_dois.json` 里，被去重逻辑排除在候选池之外，根本没走到归簇这一步，不是修复生效的证据。目前只有单元测试覆盖了这处逻辑（见 `tests/search/test_cluster.py`），还没有一次自然重跑在真实候选上验证过。
