...
- ✅ 按主题配额选稿，冷门主题不被热门主题挤没；数量不足自动用高被引论文补位（补位不要求落进当周窄主题，且换检索式最多重试 3 轮）
- ✅ **特定主题追踪**（可选）：`FOCUS_TOPIC` 填一个你自己关心的方向，AI 消化成检索式、不限日期检索、单列一节汇总——与 Zotero 库无关的方向也能追
...
│   └── focus.py                     可选的第二条检索线：把 FOCUS_TOPIC 拆成「对象组 AND 切面组」，不限日期检索
│
├── fulltext/
...
├── backfill.py               不足 `min_papers` 时用 OpenAlex 高被引论文补位；换检索式最多 3 轮（周报）
...
flowchart TD
    ...
    CHECK -->|不够| BF["OpenAlex 高被引补位<br/>（过相关度闸，但不要求<br/>属于当周某个具体主题；<br/>换检索式最多 3 轮）"]
    CHECK -->|够了| FT
    BF --> FT["抓开放获取全文<br/>结构化字段抽取"]

    FO(["FOCUS_TOPIC<br/>（可选，留空则整条线不启用）"]) --> FQ["AI 消化主题<br/>拆成对象组 + 切面组"]
    FQ --> FS["四源检索（不限日期）<br/>(对象 OR …) AND (切面 OR …)"]
    FS --> FG{"与该主题相关度<br/>≥ focus.min_relevance"}
    FG -->|不通过| DROP
    FG -->|通过| FT
...
| 特定主题（可选） | 与你指定主题不对题的（**不限日期**，检索覆盖全部年份；检索式把「对象组」与「切面组」用 AND 连接，对象是必要条件；判定用的是「与该主题相关吗」这把尺子，主题点名了具体对象时，研究别的对象的文献即使方法邻近也不放行） | `search.focus.*` |
...
库内主题是从 Zotero 语料聚出来的；`FOCUS_TOPIC` 这条线是你自己点名要追的方向，可以和库里已有的
文献毫无关系。它和主线共用抽取、渲染、去重、归档，但**选文的两个环节都不一样**：

| 环节 | 库内主题 | 特定主题 |
| --- | --- | --- |
| 检索式 | 主题的一簇相关概念 OR 在一起 | **对象组 AND 切面组**——对象是必要条件 |
| 日期 | 当周窗口（就是要看新文献） | **不限**，覆盖全部年份 |
| 相关度 | 与「生物药 CMC」的相关度（`report.min_relevance`） | 与**你给的主题**的相关度（`search.focus.min_relevance`） |
| 进「本周优先读」 | 会 | 不会（两把尺子量出来的分不可比） |

**为什么检索式要拆成两组。** 实测踩过一个不显眼的坑：主题填「乌司他丁：酶抑制活性动力学研究」时，
生成的 Europe PMC 检索式是
...
药名和「酶抑制」「动力学参数」这些通用词是 **OR** 关系——只匹配 `enzyme inhibition` 的文献照样返回，
药名等于可有可无。连着三次运行（51 篇 0 命中 / 36 篇 4 篇「方法邻近但对象不对」/ 88 篇 0 命中）都栽在这里，
而当时看上去像是"判定太严"或"窗口太窄"。现在模型必须把主题拆成 `subject_terms`（对象本身的英文名与
文献里实际使用的同义词、别名、缩写）和 `aspect_terms`（切面），合成 `(对象 OR …) AND (切面 OR …)`，
只匹配切面的文献进不来。改完之后同一个主题取回 72 篇、3 篇过线，三篇都真的是乌司他丁/尿胰蛋白酶抑制剂。

**相关度用的是另一套细则。** 你的主题可能整体就在生物药 CMC 之外，套用主线那把尺子会把你点名要的
方向全判进 20–54 档。这条线问的是「与这个主题相关吗」，且第一原则是**主题点名的对象，文献必须真的
研究它**——换了对象一律 30–59 分，方法学再邻近也不放行（细则里写死了三个真实反例：查乌司他丁时
返回的黄嘌呤氧化酶 / 酪氨酸酶 / 胆碱酯酶抑制动力学论文）。

**这一节可能是空的，那通常是对的。** 检索、去重、判定每一步的数字都会打进运行日志
（`Focus topic ...: subject [...], aspect [...]` 和 `N fetched -> N new -> N cleared relevance N`），
空了先看这两行：是压根没检索到，还是检索到了但不对题。真的没有对题文献时，空着比塞几篇
标着「非本主题」的邻近文献要好。
...
- **主题归属判定已经过多次真实运行验证：模型确实会诚实地说「无」——严格到需要重新权衡。** 上一版这里担心的是"模型会不会敷衍地挑一个最接近的主题"，四次真实运行（2026-09-01/02）给出的答案是不会：新文献候选被判「不属于任何当前主题」的比例分别是 55/60、59/60、58/60、60/60，而同期因相关度或综合分不达标被刷掉的是 **0 篇**。也就是说这道闸现在几乎独自决定了新文献能不能进周报，一期周报里真正的新文献常常只剩 1–2 篇，其余全靠经典补位撑起来。这不是 bug——每一次剔除单看都站得住脚（候选确实不属于库里那 5 个窄主题）——但它意味着两件事值得重新权衡：`search.n_clusters`（当前 5）划出的主题是不是太窄，以及这道闸对**新文献**是否也该像对补位那样放宽。经典补位已经因为同样的原因改成不要求主题归属（否则 13/13 全灭），新文献这条线还没有动。
- **特定主题这条线有两个待观察点。** 其一，模型给对象组时可能塞进有歧义的缩写——实测里「乌司他丁」的对象组包含 `UTI`，而这个缩写同样是"尿路感染"；这次 75 分门槛把噪声挡住了，但如果哪期混进不相干文献，根因大概率在这里，可以考虑在提示词里禁用歧义缩写（代价是检索面变窄）。其二，`search.focus.min_relevance` 默认 75 是按"主题点名了具体对象"这个场景定的；如果你填的是一个宽泛的方向（如"连续制造"），75 可能偏严，届时按[调参对照表](#调参对照表)下调。
